...
from tests.unit_testing.telemetry.test_prescription import days_ago
from unit_testing.telemetry.patient import Patient
from unit_testing.telemetry.prescription import Prescription
    ...
    def test_clash_overlapping_today(self):
        patient = Patient(prescriptions=[Prescription("Codeine",
                                         ...
                                         Prescription("Prozac",
                                                      dispense_date = days_ago(days=2),
                                                      days_supply=3)])
        self.assertSetEqual({days_ago(days=2),
                             days_ago(days=1),
                             days_ago(days=0)},
                            patient.clash(["Codeine", "Prozac"]))
''